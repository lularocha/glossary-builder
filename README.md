...
Glossary Builder uses a simple workflow: choose your UI language, enter a seed word (with optional title for context), and the AI generates 12 related terms with clear definitions. Each term can be expanded for deeper context with sources from official documentation. Export your glossary as markdown or Word document, and everything auto-saves to your browser.
...
3. Create a `.env` file in the root directory and set the provider you want to use (only the matching API key is required):

   ```env
   # "deepseek" (default), "gemini", or "anthropic"
...
2. Enter an optional **title** to provide context
   ...
   - Helps the AI understand the specific domain
...
- **Multi-provider LLM integration** - DeepSeek (default), Gemini, or Claude Sonnet, selected via the `LLM_PROVIDER` env var (see `api/_llm.ts`)
...
The AI automatically detects the language of your seed word and generates the glossary in that language:

- Enter "API" → English glossary
- Enter "API" with Portuguese context → Portuguese glossary
- Works with any language the model supports
...
2. Add `LLM_PROVIDER` (`deepseek`, `gemini`, or `anthropic`) and the matching API key (`DEEPSEEK_API_KEY`, `GEMINI_API_KEY`, or `ANTHROPIC_API_KEY`) to your Vercel project's environment variables
3. Deploy - Vercel automatically detects and deploys the API functions

The `api/` directory contains Vercel Serverless Functions that handle LLM API calls server-side, keeping your API key secure.
...
- Verify `.env` file exists with `LLM_PROVIDER` set and a valid API key for that provider
- DeepSeek keys require a topped-up balance (prepaid) — check platform.deepseek.com
...
- Verify your provider API key is valid
